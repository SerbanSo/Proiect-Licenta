...
def predict(_run, _log):
    ...
    floor_mask = floor_mask.astype(np.int64)
    ...
    with torch.no_grad():
        ...
        predict_floor = np.multiply(floor_mask, predict_segmentation + 1)
        # np.savetxt('test2.csv', predict_floor, delimiter=',')
        # print(np.bincount(predict_floor.flatten()))
        floor_value = np.argmax(np.bincount(predict_floor.flatten())[1:]) + 1
        predict_floor[predict_floor != floor_value] = 20
        predict_floor[predict_floor == floor_value] = 0
        ...
        # Save the plane segmentation
        # cv2.imwrite('prediction/pred_seg.png', pred_seg)

        # Save the depth prediction
        # cv2.imwrite('prediction/depth.png', depth)

        # Can't imshow on wsl (no display)
